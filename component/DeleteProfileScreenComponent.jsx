...
const DeleteProfileScreenComponent = ( {navigation} ) => {
  ...
  const deleteProfile = async() => {
    ...
    //console.log(serverUrl + '/profile/deletePersonalData')

    
    try {
      const response = await fetch(serverUrl + '/profiles/deletePersonalData', {
          method: 'GET',
          headers: {
      ...
      if (!response.ok) {
        ...
        if(json.message==="success"){
          setValidationError("Successfully deleted personal data.");
        } 
      }
    } catch (error) {
      console.log("Error");
  ...
  return (
    <ScrollView>
      <View style={styles.container}>
        ...
        <Text style={styles.headerTitle}>{translate('delete_personal_data_title')}</Text>
        <Text style={styles.text}>{translate('delete_profile_text')}</Text>
        <TouchableOpacity style={styles.submitButton} onPress={() => deleteProfile() }>
          <Text style={styles.submitButtonText}>{translate('delete_profile_button')}</Text>
        </TouchableOpacity>
